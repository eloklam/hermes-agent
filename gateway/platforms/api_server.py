...
class APIServerAdapter(BasePlatformAdapter):
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Use child session ID if created, otherwise generate new UUID
        session_id = child_session_id or str(uuid.uuid4())

        # AUTO-CONTEXT LOADING: If this is a delegated session, search for previous context
        if session_id.startswith("delegated-"):
            try:
                from hermes_state import SessionDB

                # Extract source profile from session_id
                parts = session_id.split("-")
                if len(parts) >= 2:
                    source_profile_from_id = parts[1]

                    db = SessionDB()
                    all_sessions = db.list_sessions_rich()

                    # Find recent delegated sessions from same source (excluding current)
                    recent_sessions = [
                        s
                        for s in all_sessions
                        if s.get("id", "").startswith(
                            f"delegated-{source_profile_from_id}-"
                        )
                    ...
                    if recent_sessions:
                        ...
                        prev_session_id = most_recent["id"]

                        # Load messages from that session
                        prev_messages = db.get_messages(prev_session_id)
                        if prev_messages:
                            ...
                            if system_prompt:
                                system_prompt = context_summary + system_prompt
                            else:
                                system_prompt = context_summary

                            logger.info(
                                f"Auto-loaded context from {prev_session_id} "
                                f"({len(prev_messages)} messages) for session {session_id}"
                            )
            except Exception as e:
                logger.debug(f"Auto-context loading failed: {e}")
